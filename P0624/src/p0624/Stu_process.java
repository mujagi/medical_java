...
public class Stu_process {
	...
	void stu_output(Stu_score[] sc){
		...
		for(int i=0;i<s_count;i++) {
			...
			System.out.printf("%d\t",sc[i].kor);  //이름
			System.out.printf("%d\t",sc[i].eng);  //이름
			System.out.printf("%d\t",sc[i].math);  //이름
			System.out.printf("%d\t",sc[i].total);  //이름
			System.out.printf("%.2f\t",sc[i].avg );  //이름		
			System.out.printf("%d\t",sc[i].rank );  //이름		
			System.out.println();
		}
		System.out.println();
	}// stu_output
	...
	void stu_rank(Stu_score[] sc) {
		...
		for(int i=0;i<s_count;i++) {
			...
			for(int j=0;j<s_count;j++) {
				if(sc[i].total<sc[j].total) { // 각각 비교  해야므로 i,j 따로 부
					count++;
				}// if
				sc[i].rank = count;
			}// j for
