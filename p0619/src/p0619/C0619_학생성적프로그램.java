...
public class C0619_학생성적프로그램 {

	public static void main(String[] args) {
		// 학생성적
		Scanner scan = new Scanner(System.in);
		//3명 학생 - 국어,영어,수학
		final int STU = 10; // 전체 학생 수
		...
		Loop1:while(true) {
			...
			case 1:
				...
				while(true) {
					...
					//0001
         			...
         			System.out.printf("학번 : %s",stuNo[s_count]);
         			System.out.println();
			...
			case 3:
				...
				//검색
				int temp_no = -1; // i와 겹치기 때문에
				for(int i=0; i<s_count;i++) {
					if(name[i].equals(search)) {
						temp_no = i; // 이거랑 // 찾는 학생이 검색됬을 경우
				...
				if(temp_no==-1) { // 찾는 학생이 없을 경우
					...
					case 1 :
						...
						System.out.printf("국어점수가 %d로 변경되었습니다. \n",score[temp_no][choice-1]);
						break;
					case 2 :
						System.out.printf("[ %s점수 수정 ] \n",title[choice+1]);
						System.out.printf("현재 %s점수 :  %d \n",title[choice+1],score[temp_no][1]);
						System.out.println("----------------------------------------");
						System.out.printf("변경 %s점수를 입력하세요. >>  \n",title[choice+1]);
						score[temp_no][choice-1] = scan.nextInt();
